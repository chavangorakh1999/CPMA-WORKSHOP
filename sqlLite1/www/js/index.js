var createStatement =
  "CREATE TABLE IF NOT EXISTS Contacts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, useremail TEXT)";
...
var updateStatement =
  "UPDATE Contacts SET username = ?, useremail = ? WHERE id=?";
...
function updateRecord() {
  ...
  db.transaction(function (tx) {
    tx.executeSql(
      updateStatement,
      [usernameupdate, useremailupdate, Number(useridupdate)],
      loadAndReset,
      onError
    );
  });
...
function showRecords() {
  ...
  //I WANT THIS TO CLEAR TO DIPLAY ALL RECORDS INTO IT
  $("#results").html("");
...
{
  $("body").fadeIn(3000); // Fede In Effect when Page Load..
